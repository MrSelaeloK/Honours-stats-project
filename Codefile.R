...
African_Dusky_Flycatcher_validation_results_Grid_1 = read_csv("African Dusky Flycatcher validation results Grid 1.csv")
African_Dusky_Flycatcher_validation_results_Grid_1 = rename(African_Dusky_Flycatcher_validation_results_Grid_1,score = confidence)


# -------Chunk 2 - Call density estimation-------------------------------------
#binning and weighting are calculated within the function
Study_level_call_density = function(examples,num_beta_samples=1000,small_constant=0.1){
  ...
  df = as.data.frame(do.call(rbind, examples), stringsAsFactors = FALSE)
...
#Density  
  ...
  for(i in 1:nrow(shape.parameters)){
    density_matrix[i,1] = rbeta(1,shape.parameters[i,1]+ small_constant,
                             shape.parameters[i,2] + small_constant)*bin_weights[i,1]
  }
...
out = Study_level_call_density(African_Dusky_Flycatcher_validation_results_Grid_1,1000,0.1)  
...
#sum(P(+|b)*P_s(b))
Strat1_call_density <- function(examples, num_beta_samples=1000, small_constant=0.1){
  ...
  logit.scores = sapply(unlist(df$score), function(b) log(b)-log(1-b))            #generating logit scores
  df$score = logit.scores                                                         #turning scores to logit scores
  examples = as.data.frame(do.call(rbind, fdata),stringsAsFactors = FALSE)        #Conversion to data-frame
  examples = rename(examples,binNumber=bin, outcome = is_pos)                     #renaming of columns
  ...
  for(i in 1:nrow(out$beta_parameters)){
    density_matrix[i,1] =  rbeta(1,out$beta_parameters[i,1]+small_constant,
                              out$beta_parameters[i,2]+small_constant)*bin_weights[i,1]
  }
  density = sum(density_matrix)
...
#---------Strategy 2------------------------------------------------------------
Strat2_call_density = function(site_examples, study_fit = out, eps = 1e-12,small_constant= 0.1) {
  
  
  #site_examples = study_level_data
  ...
  for(i in 1:B){
    site_level_densities[i,1] = rbeta(1,site_bin_positive[i,2]+small_constant,
                                      site_bin_negative[i,2]+small_constant)*site_bin_weight[i]
  }
  ...
  #experimentation with new technique
  #df = as.data.frame(do.call(rbind, study_level_data), stringsAsFactors = FALSE)
  #quantile_bounds = seq(0,1,length.out = bins + 1)
  #bin_indices = findInterval(unlist(df$score),quantile_bounds,rightmost.closed = TRUE)
  #study_level_bin_weights = table(bin_indices) / nrow(df) 
  #Ps_b =(study_level_bin_weights)
...
bootstrap_strat2_site = function(site_examples, study_fit,Bdraws = 2000,conf = 0.95,eps = 1e-12) {
  ...
  point = Strat2_call_density(site_examples, study_fit = study_fit, eps = eps)
  boots = Bdraws
  
  for (b in seq_len(Bdraws)) {
    idx = sample.int(n, n, replace = TRUE)
    boots[b] = Strat2_call_density(site_examples[idx], study_fit = study_fit, eps = eps)
  }
...
bootstrap_strat3_site = function(site_examples,study_fit,Bdraws = 2000,conf = 0.95,eps = 1e-12) {
  ...
  for (b in seq_len(Bdraws)) {
    s1 = Strat1_call_density(fdata,10000,0.1)$Density_Estimate
    s2 = Strat2_call_density(fdata,study_fit = out,eps = 1e-12, small_constant = 0.1)
    boots[b] = sqrt(s1 * s2)
  }
...
out3.1 = Strat3_call_density(Strat1_call_density(fdata,num_beta_samples=1000, small_constant=0.1)$Density_Estimate,
                    Strat2_call_density(fdata,study_fit = out,eps = 1e-12, small_constant = 0.1))

out3 = bootstrap_strat3_site(fdata,out,Bdraws = 200,conf = 0.95,eps = 1e-12)
hist(out3$samples)
out3.1
